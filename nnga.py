...
def genetic_algorithm(pop_size, generations, mutation_rate, weight_length, env1, env2, run_id):
    ...
    max_fitness_over_gens = []

    for gen in range(generations):
        # Evaluate fitness of individuals on both environments
        fitness_scores = np.array([evaluate_individual(ind, env1, env2) for ind in population])

        # Calculate mean and max fitness for this generation
        mean_fitness = np.mean(fitness_scores)
        max_fitness = np.max(fitness_scores)
        mean_fitness_over_gens.append(mean_fitness)
        max_fitness_over_gens.append(max_fitness)
    ...
    # Save the mean and max fitness across generations for this run
    np.savetxt(f'{results_folder}/ga_mean_fitness_run{run_id}.txt', mean_fitness_over_gens)
    np.savetxt(f'{results_folder}/ga_max_fitness_run{run_id}.txt', max_fitness_over_gens)

    return population[np.argmax(fitness_scores)]  # Return the best solution


def evaluate_individual(weights, env1, env2):
    # Run fitness evaluation on both environments
    fitness1 = env1.play(pcont=weights)[0] if env1.play(pcont=weights) else 0
    fitness2 = env2.play(pcont=weights)[0] if env2.play(pcont=weights) else 0
    return (fitness1 + fitness2) / 2  # average fitness across both environments
